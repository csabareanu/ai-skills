# Lesson [Number] — [Title]

- Status: `[draft, in progress, completed, or revised]`
- Roadmap focus:
- Module:
- Originally taught: `[YYYY-MM-DD or not yet taught]`
- Last revised: `[YYYY-MM-DD]`

## Outcome

State the coherent capability this lesson develops.

## Prerequisites And Course Connection

- Expected entry knowledge:
- Earlier lessons used:
- What this lesson enables next:

## Why This Matters

Connect the lesson to a meaningful question, performance, or later demand.

## Pre-Delivery Theory Gate

Complete this authoring gate before beginning independent practice on new
material. Mark an item not applicable only with a brief, lesson-specific
reason.

- [ ] Central idea or problem
- [ ] Governing model or mechanism
- [ ] Meaningful connection, comparison, or alternative
- [ ] Limits, consequences, or failure modes
- [ ] Worked example, demonstration, case, or source encounter
- [ ] For programming lessons: design rationale, semantic and compiler or
  runtime behavior, and a genuine alternative or tradeoff

Do not begin independent practice until every relevant item is visibly taught
in the conversation and preserved below.

## Big Idea And Intellectual Context

Explain the central problem or abstraction and any relevant intellectual,
historical, or design context. Include context for explanatory value, not as
decoration.

## Key Vocabulary

Define only terms needed for the lesson and connect them to one another.

## Theory, Mechanism, And Mental Model

Present a coherent explanation that can be understood without the original
conversation. Explain what happens and why, not merely the visible notation or
steps. Include important assumptions and relationships.

## Connections, Comparisons, And Alternatives

Connect the model to prior knowledge and compare genuine alternatives,
including their semantic differences and tradeoffs. Omit comparisons that do
not improve understanding.

## Disciplinary Lenses And Interpretations

Use the subject's relevant lenses to examine causes, structures,
interpretations, consequences, or disputes. Examples include philosophical,
sociological, economic, political, historical, scientific, aesthetic,
linguistic, ethical, and technical perspectives. Select lenses for explanatory
power rather than filling a checklist.

## Worked Examples Or Demonstrations

Preserve representative examples actually discussed. Clearly distinguish
tutor-authored examples from learner-authored artifacts.

## Common Confusions, Limits, And Failure Modes

Record reusable cautions and corrected models, not a transcript of learner
mistakes.

## Practice And Self-Check

Include retrieval questions, transfer prompts, or practice worth revisiting.
Do not expose solutions to an active independent assessment.

## What To Remember

Summarize the smallest set of durable ideas and relationships worth retrieving
later.

## References And Related Artifacts

- Sources:
- Learner artifacts:
- Session and evidence records:

## Revision Log

| Date | Revision | Relationship to original lesson |
| --- | --- | --- |
| [YYYY-MM-DD] | [Correction, clarification, example, or scope change] | [Originally taught or added later] |
