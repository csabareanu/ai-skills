---
name: adopt
description: Bring the blueprint into an existing (brownfield) codebase. Surveys the real repo, interviews for intent, generates the owned plans and coding standards, documents existing verification and CI, and leaves optional CI setup as a separate project-specific task. Use when the user runs /adopt, is overlaying the blueprint onto an app that already has meaningful code, or asks to adopt or bootstrap the workflow into an existing project. For freshly scaffolded or early projects, use onboard instead.
---
...
pull-request checks are absent, note that CI setup is optional and outside this
fork. It can be added later as a separate, project-specific task; CI is not
required to finish adoption.
